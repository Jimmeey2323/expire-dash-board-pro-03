...
class GoogleSheetsService {
  ...
  async saveAnnotation(uniqueId: string, memberId: string, email: string, comments: string, notes: string, tags: string[], noteDate: string): Promise<void> {
    try {
      ...
      // Use unique ID from column A as primary key for matching
  ...
  async getMembershipData() {
    try {
      ...
      // Create annotation map using unique ID from column A as primary key
      ...
      return rows.map(row => {
        ...
        // Find annotations using unique ID from column A
        const annotations = annotationsMap.get(uniqueId) || {
          comments: '', 
          notes: '', 
          tags: [], 
          noteDate: ''
        ...
        // Create additional identifiers for backup matching
        const persistenceKey = `${uniqueId}-${memberId}-${email}`.toLowerCase();
        ...
        return {
          ...
          // Additional fields with proper date formatting
          startDate: this.formatDate(row[10]) || '', // Using order date as start date
          totalSessions: parseInt(row[8]) || 0, // Default to sessions left if no total
